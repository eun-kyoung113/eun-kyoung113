...
pdf(file=plotsPath,width=30000,height=25000)  
...
for(i in 1:length(CAL2_level)){
  ...
  lv1_leukemia_CAL2$simple.reg[ind.val]<-exp(predict(model.simple.reg,newdata=lv1_leukemia_CAL2[ind.val,]))
...
for(i in 1:length(SEX_CAL2_level)){
  ...
  lv1_leukemia_SEX_CAL2$poisson.reg[ind.tr]<-model.simple.reg$fitted.values
  lv1_leukemia_SEX_CAL2$poisson.reg[ind.val]<-exp(predict(model.simple.reg,newdata=lv1_leukemia_SEX_CAL2[ind.val,]))
...
for(i in 1:length(name_level)){
  ...
  model.linear.spline.FY<-glm(sum_FY_BZ~bs(YEAR,knots=c(2005,2010),degree=1),data=lv2_leukemia_CAL2[ind.tr,])
...
for(i in 1:length(name_level)){
  ...
  lv2_leukemia_SEX_CAL2$simple.reg[ind.tr]<-model.poisson.reg$fitted.values
  lv2_leukemia_SEX_CAL2$simple.reg[ind.val]<-exp(predict(model.poisson.reg,newdata=lv2_leukemia_SEX_CAL2[ind.val,]))
...
for(i in 1:length(name_level)){
  ...
  model.linear.spline<-glm(sum_LEUKEMIA_BZ~bs(YEAR,knots=c(2005,2010),degree=1),data=lv3_leukemia_total[ind.tr,],family='poisson')
  lv3_leukemia_total$linear.spline[ind.tr]<-model.linear.spline$fitted.values
  lv3_leukemia_total$linear.spline[ind.val]<-exp(predict(model.linear.spline,newdata=lv3_leukemia_total[ind.val,]))
  
  model.linear.spline.FY<-glm(sum_FY_BZ~bs(YEAR,knots=c(2005,2010),degree=1),data=lv3_leukemia_total[ind.tr,],family='poisson')
  lv3_leukemia_total$linear.spline.FY[ind.tr]<-model.linear.spline.FY$fitted.values
  lv3_leukemia_total$linear.spline.FY[ind.val]<-exp(predict(model.linear.spline.FY,newdata=lv3_leukemia_total[ind.val,]))
...
ggplot()+geom_line(data=lv3_leukemia_CAL2[lv3_leukemia_CAL2$CAL2=="234",],aes(x=YEAR,y=sum_FY_BZ,color="red"),size=1)+labs(y="#of FY BZ")+
  ...
  geom_text(size=3,data=as.data.frame(MAPE_lv3_leukemia_234),mapping = aes(x = 2003, y = Inf, label = linear_spline.FY),vjust=1.5,colour="purple",fontface="bold")+
  geom_text(size=3,data=as.data.frame(MAPE_lv3_leukemia_234),mapping = aes(x = 2003, y = Inf, label = quadratic_reg.FY),vjust=3,colour="brown1",fontface="bold")+
  labs(x="Year")+
  scale_color_identity(name='#of FY BZ',
                       breaks=c("red","black","green","darkviolet","deeppink1"),
                       labels=c("true value of leukemia","simple linear regression","poisson regression","linear spline","quadratic regression"),
...
for(i in 1:length(CAL2_level)){
  ...
  lv1_lung_CAL2$simple.reg[ind.val]<-exp(predict(model.simple.reg,newdata=lv1_lung_CAL2[ind.val,]))
...
for(i in 1:length(SEX_CAL2_level)){
  ...
  lv1_lung_SEX_CAL2$poisson.reg[ind.tr]<-model.simple.reg$fitted.values
  lv1_lung_SEX_CAL2$poisson.reg[ind.val]<-exp(predict(model.simple.reg,newdata=lv1_lung_SEX_CAL2[ind.val,]))
...
for(i in 1:length(name_level)){
  ...
  model.linear.spline.FY<-glm(sum_FY_BZ~bs(YEAR,knots=c(2005,2010),degree=1),data=lv2_lung_CAL2[ind.tr,])
...
for(i in 1:length(name_level)){
  ...
  lv2_lung_SEX_CAL2$simple.reg[ind.tr]<-model.poisson.reg$fitted.values
  lv2_lung_SEX_CAL2$simple.reg[ind.val]<-exp(predict(model.poisson.reg,newdata=lv2_lung_SEX_CAL2[ind.val,]))
...
for(i in 1:length(name_level)){
  ...
  model.linear.spline<-glm(sum_LUNG_BZ~bs(YEAR,knots=c(2005,2010),degree=1),data=lv3_lung_total[ind.tr,],family='poisson')
  lv3_lung_total$linear.spline[ind.tr]<-model.linear.spline$fitted.values
  lv3_lung_total$linear.spline[ind.val]<-exp(predict(model.linear.spline,newdata=lv3_lung_total[ind.val,]))
  
  model.linear.spline.FY<-glm(sum_FY_BZ~bs(YEAR,knots=c(2005,2010),degree=1),data=lv3_lung_total[ind.tr,],family='poisson')
  lv3_lung_total$linear.spline.FY[ind.tr]<-model.linear.spline.FY$fitted.values
  lv3_lung_total$linear.spline.FY[ind.val]<-exp(predict(model.linear.spline.FY,newdata=lv3_lung_total[ind.val,]))
...
ggplot()+geom_line(data=lv3_lung_SEX_CAL2[lv3_lung_SEX_CAL2$SEX=="F" & lv3_lung_SEX_CAL2$CAL2=="234",],aes(x=YEAR,y=sum_FY_BZ,color="red"),size=1)+labs(y="#of FY BZ")+
  ...
  theme(legend.title=element_text(size=13),
        legend.text=element_text(size=12))
dev.off() 
value_eklee_lv3<-as.data.frame(rbind(lv3_lung_total[2:length(lv3_lung_total)],lv3_lung_SEX[3:length(lv3_lung_SEX)],lv3_lung_CAL2[3:length(lv3_lung_CAL2)],
...
MAPE.df<-merge(MAPE_leukemia_total,MAPE_lung_total,key="NAME",all=TRUE)
RMSE.df<-merge(RMSE_leukemia_total,RMSE_lung_total,key="NAME",all=TRUE)
