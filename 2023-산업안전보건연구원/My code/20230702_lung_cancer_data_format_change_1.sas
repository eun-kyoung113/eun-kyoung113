...
proc sql;
create table check1 as select distinct Outcome_Category from mine.lung_raw
where between_yes=1;
quit;

proc sql;
create table check2 as select INDI_ID, ECNY_DT, OUT_DT, Outcome_Category, Outcome_date from mine.lung_raw
where INDI_ID not in (select distinct INDI_ID from mine.lung_raw where between_yes=1) and Outcome_Category='lung_cancer';
quit;
