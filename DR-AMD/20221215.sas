...
proc sql;
create table check_event as select Patient_ID,  Eye, SUM(bad) as bad_check, SUM(prog) as prog_check, SUM(pdr) as pdr_check from mydata_ver0
...
proc sort data=mydata_ver0;
by Patient_ID Eye;
run;

DATA event_data;
MERGE mydata_ver0 check_event;
BY Patient_ID Eye;
DROP FU_Duration2;
run;
...
/* Check the record which FU_Duration <0 and FU_Duration >-1 */
...
SET new_ver2;
BY Patient_ID Eye;
IF (last.Eye) & (FU_Duration<0) & (FU_Duration>-1);
...
SET new_ver2;
BY Patient_ID Eye;
IF (last.Eye) & (FU_Duration<0) & (FU_Duration>-1) then DELETE;
