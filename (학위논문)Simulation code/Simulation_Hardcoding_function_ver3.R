...
weight_make<-function(var_treat,var_cov,estimate,data,formula){
  result<-list()
...
DR_estimator<-function(estimate,data,var_treat,var_y,cov_outcome,cov_ps,ps_formula,outcome_formula){
  
  result<-list()
  mydata<-data[c(var_treat,cov_ps)]
  
  #ps_formula<-readline("Write the formula of treatment model ")
  myformula<-as.formula(ps_formula)
  fit<-glm(formula=myformula,data=mydata,family='binomial')
  result$fit<-fit
  ...
  #outcome_formula<-readline("Write the formula of Outcome model ")
  myformula1<-as.formula(outcome_formula)
  
  #ind_mu0<-which(data[,var_treat]==0)
  #mu0_df<-data[ind_mu1,c(var_y,cov)]
  
  #data2<-data[c(var_y,cov_outcome)]
  data2<-data[c(var_y,var_treat,cov_outcome)]
  
  out_model<-lm(formula=myformula1,data=data2)
  new_dt_mu0<-data2
  new_dt_mu0[,var_treat]<-0
  mu0_X<-predict(out_model,newdata=new_dt_mu0)
  ...
  if(estimate=='ATE'){
    ...
    new_dt_mu1<-data2
    new_dt_mu1[,var_treat]<-1
    mu1_X<-predict(out_model,newdata=new_dt_mu1)
    
    result$mu1_X<-mu1_X
    
    mu1_dr<-mean(data[,var_treat]*data[,var_y]/ps - ((data[,var_treat]-ps)/ps)*(mu1_X))
    mu0_dr<-mean(mu0_X+((1-data[,var_treat])*(data[,var_y]-mu0_X))/(1-ps))
    ...
    #ind_mu1<-which(data[,var_treat]==1)
    result$mu1_dr<-sum(data[,var_y]*data[,var_treat])/sum(data[,var_treat])
    
    w.hat<-ps/(1-ps)
    result$est<-sum((data[,var_treat]-(1-data[,var_treat])*w.hat)*(data[,var_y]-mu0_X))/sum(data[,var_treat])
  }
...
DR_Naive_var_estimator<-function(estimate,data,var_treat,var_y,cov_outcome,cov_ps,ps_formula,outcome_formula){
  result<-DR_estimator(estimate,data,var_treat,var_y,cov_outcome,cov_ps,ps_formula,outcome_formula)
  ...
  if(estimate=="ATE"){
    ...
    w.hat<-fit$fitted.values/(1-fit$fitted.values)
    tau_i<-(data[,var_treat]-(1-data[,var_treat])*w.hat)*(data[,var_y]-result$mu0_X)
...
### -------------------------------------------------------------------------
### 2)-2 Sandwich robust variance of DR estimator -------------------------------------
sandwich_var_DR<-function(estimate,data,var_treat,var_y,cov_outcome,cov_ps,ps_formula,outcome_formula){
  ...
  outcome_data<-model.matrix(result$out_model)
  X_1<-X_0<-outcome_data
  ...
  if(estimate=="ATE"){
    ...
    for(i in 1:nrow(data)){
      ...
      nu1_4<-matrix(as.vector((data[,var_treat][i]*(data[,var_y][i]- result$mu1_X[i]))/exp_beta_X[i])*model.matrix(fit)[i,],nrow=1)
      nu1<-cbind(1,0,nu1_3,nu1_4)
      colnames(nu1)<-NULL
      
      nu2_3<-matrix(((-(1+exp_beta_X[i]))*data[,var_treat][i]+exp_beta_X[i])*X_0[i,],nrow=1)
      nu2_4<- as.vector((-1)*exp_beta_X[i]*((1-data[,var_treat][i])*(data[,var_y][i]-(result$mu0_X[i]))))*model.matrix(fit)[i,]
      nu2<-cbind(0,1,matrix(nu2_3,nrow=1),matrix(nu2_4,nrow=1))
      colnames(nu2)<-NULL
      
      nu3_3<--outcome_data[i,]%*%t(outcome_data[i,])
      nu3<-cbind(0,0,nu3_3,
                 matrix(0,ncol=length(coef(fit)),nrow=dim(t(outcome_data)%*%outcome_data)[1]))
      colnames(nu3)<-NULL
    ...
    ### pi-hat ###
    est_1_hat<-result$mu1_dr-((data[,var_treat]*data[,var_y]-(data[,var_treat]-fit$fitted.values)*(result$mu1_X))/fit$fitted.values)
    est_2_hat<-(result$mu1_dr-result$est)-(((1-data[,var_treat])*data[,var_y]+(data[,var_treat]-fit$fitted.values)*(result$mu0_X))/(1-fit$fitted.values))
    est_3_hat<-as.vector(data[,var_y] - result$mu1_X)*outcome_data
    est_4_hat<-as.vector(data[,var_treat]-fit$fitted.values)*model.matrix(fit)
    
    estimating_equation_true <- as.matrix(cbind(est_1_hat,est_2_hat,est_3_hat,est_4_hat)) # n by 9
...
### -----------------------------------------------------
### 2)-3 J_function -----------------------------------------------
J_function<-function(data,data_L,var_treat,var_y,cov_outcome,cov_ps,ps_formula,outcome_formula){
  ...
  DR_ATT<-DR_estimator("ATT",data,var_treat,var_y,cov_outcome,cov_ps,ps_formula,outcome_formula)
  fit<-DR_ATT$fit
  X_P<-model.matrix(fit)
  X_out<-model.matrix(DR_ATT$out_model)
  ...
  for(i in 1:nrow(data_L)){
    ...
    nu2_4<- ((-1)*nrow(data_L)/sum(data_L[,var_treat]==1))*(as.vector(exp_beta_X[i]*((1-data_L[,var_treat][i])*(data_L[,var_y][i]-(DR_ATT$mu0_X[i]))))*X_P[i,])
    nu2<-cbind(0,1,matrix(nu2_3,nrow=1),matrix(nu2_4,nrow=1))
    colnames(nu2)<-NULL
...
### ----------------------------------------------------------------------------
### 2)-4 crossprod of estimating_equation function -------
crossprod_est_equ<-function(data,data_L,var_treat,var_y,cov_outcome,cov_ps,ps_formula,outcome_formula){
  ...
  est_2_true<-nu0-(nrow(data_L)/sum(data_L[,var_treat]==1))*((((1-data_L[,var_treat])*data_L[,var_y]*fitted_ps+(DR_ATT$mu0_X)*(data_L[,var_treat]-fitted_ps)))/(1-fitted_ps))
  est_3_true<-as.vector(data_L[,var_y]-DR_ATT$out_model$fitted.values)*X_out
  est_4_true<-as.vector(data_L[,var_treat]-fitted_ps)*X_P
  
  estimating_equation_true <- as.matrix(cbind(est_1_true,est_2_true,est_3_true,est_4_true)) # n by 9
