...
|-|-|-|-|-|-|
|2024.05~현재 진행 중|${\star}$ 사용자 로그 데이터 <br> ${\star}$ Longitudinal Data|App 사용자 로그 데이터 분석|**담당 실무자** <br> ${\cdot}$ 데이터 EDA 및 분석 <br> ${\cdot}$ data schema 생성|${\surd}$ Raw data 파악 및 정제 역량 <br> ${\surd}$ Python program 능력(데이터 전처리)|excel file로 결과 전달 예정| 
|2024.03~현재 진행 중중|${\star}$ 추천 알고리즘 기획 및 구축 <br> ${\star}$ DB 설계|건강 유형 별, 맞춤 건강 기능 식품 추천 알고리즘 고도화|**담당 실무자** <br> ${\cdot}$ DB 통합, 알고리즘 기획 및 구현 <br> ${\cdot}$ DB 설계 support|${\surd}$ Python program 능력 <br> ${\cdot}$ 두 개의 DB 통합 및 홈페이지 crawling <br> ${\surd}$ 알고리즘 기획 및 구현 <br> ${\surd}$ Data 구축 및 정제 역량|자사 앱 및 타사 납품 예정|
|2024.01~2024.02|${\star}$ Data EDA 및 통계량 추출|**메리츠 화재 의뢰** 과제 - 추적 관찰 기간 동안, 경증 질환 발병 관련 통계량 추출|**담당 실무자** <br> ${\cdot}$ 데이터 전처리 및 통계량 추출 <br> ${\cdot}$ 의사소통 담당|${\surd}$ SAS & SQL program 능력 <br> ${\cdot}$ 건강보험 청구 자료 전처리 및 통계량 추출 <br> ${\surd}$ code pipeline 구축 및 결과 reporting 역량|${\cdot}$ 최종 결과 excel file로 전달 <br> ${\cdot}$ **추가 계약 달성**|
