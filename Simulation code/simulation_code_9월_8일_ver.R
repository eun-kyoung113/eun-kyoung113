########### Simulation code 9/8 version ##############
####### Update #########
## 1. DR Sandwich Variance estimator coding
########################
...
sandwich_var_DR<-function(estimate,data,var_treat,var_y,cov){
  ...
  if(estimate=="ATE"){
    ...
    for(i in 1:nrow(data)){
      ...
      nu4_4<- -(fit$fitted.values[i]*(1/(1+exp_beta_X[i])))*(model.matrix(fit)[i,]%*%t(model.matrix(fit)[i,]))
    ...
    for(j in 1:nrow(data)){
      ...
      nu1_hat<-result$mu1_dr-((data[,var_treat][1]*data[,var_y][1]-(data[,var_treat][1]-fit$fitted.values[1])*(coef(result$out_model)%*%X_1[1,]))/fit$fitted.values[1])[1]
      nu2_hat<-(result$mu1_dr-result$est)-(((1-data[,var_treat][1])*data[,var_y][1]-(data[,var_treat][1]-fit$fitted.values[1])*(coef(result$out_model)%*%X_0[1,]))/(1-fit$fitted.values[1]))[1]
      nu3_hat<-as.matrix((result$out_model$residuals[1])*model.matrix(result$out_model)[1,],ncol=1)
      nu4_hat<-as.matrix((1-fit$fitted.values[1])*model.matrix(fit)[1,],ncol=1)
    ...
    var_est<-(cov_var_matrix[1,1]+cov_var_matrix[2,2]-2*cov_var_matrix[1,2]) / nrow(data)
