#### 5/13 update ver ####
## 1. result_table write function update / Confidence interval & coverage calculation function writing
## 2. robust variance calculation function update - IPTW ATE
...
sandwich_var<-function(obj,var_treat,var_weight,data){
  
  first_term<-matrix(c(0,0,0,0),nrow=2)
  second_term<-matrix(c(0,0,0,0),nrow=2,ncol=2)
  pearson_resid<-residuals(obj,type="pearson")
  pi<-sum(pearson_resid^2)/(nrow(data)-2)
  
  for(i in 1:nrow(data)){
    S_i<-obj$residuals[[i]]
    V_i<-pi
    #V_i<-pi*((S_i/pearson_resid[i])^2)
    D_i<-matrix(c(1,data[,var_treat][i]),nrow=1)
    value_first<-(t(D_i)%*%D_i)/V_i
    #value_first<-(data[,var_weight][i]/sum(data[,var_weight]))*(t(D_i)%*%D_i)/V_i
    value_second<-(t(D_i)%*%D_i)*(S_i^2)/(V_i^2)
    
    first_term<-first_term+value_first
    second_term<-second_term+value_second
  }
  
  #result<-sum(term)/nrow(data)
  result<-solve(first_term)*(second_term)*solve(first_term)
  #result<-second_term/nrow(data)
  return(result)
}
################################################

## result writing function ##
result_table<-function(obj,var_treat,var_weight,cov,cov_type,data){
  result_table<-rep(0,4)
    ...
    robust_var<-sandwich_var(obj,var_treat,var_weight,data)
...
sandwich_var(ATE_untrimmed,"E","weight_untrimmed",data)
...
CI_coverage_make<-function(result_table,var_estimator,var_SE,true_value){
  ...
  SE_estimator<-result_table[,var_estimator]
