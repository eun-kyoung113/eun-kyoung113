# Simulation code 10/12 Version --------------------------------------------
### <What to Do> ------------------------------------------------------------
## 1. Debugging Sandwich Variance Estimator function of DR ATE Estimator --- TO DO LIST UPDATE
### -------------------------------------------------------------------------
...
### 2-3) Debugging Variance estimator of DR ATT estimator ----------------------------------
...
### 2-3-1-3) 1000th Replication--------------
theta_hat_check<-matrix(0,nrow=9,ncol=1000)
theta_true_check<-matrix(0,nrow=9,ncol=1000)
dim1<-9
...
for(i in 1:1000){
  ...
  nu_theta_total<-apply(nu_theta_array,c(1,2),mean)
  
  two<- -sqrt(nrow(data))*nu_theta_total%*%(theta_hat-theta_true)
  ...
  nu_true_total<-apply(nu_true_array,c(1,2),mean)
  
  three<--sqrt(nrow(data))*nu_true_total%*%(theta_hat-theta_true)
  
  
  
  
  
  ### estimating equation_theta_true ------------------------------------------
  ...
  z_true<-apply(estimating_equation_true,1,sum)
  
  one<-matrix(z_true/sqrt(nrow(data)),ncol=1)
  
  theta_hat_check[,i]<-abs(one-two)
  theta_true_check[,i]<-abs(one-three)
  
}

apply(theta_hat_check,1,mean)
apply(theta_true_check,1,mean)
