#### 5/20 update ver ####
## 1. robust variance calculation function update - IPTW ATE
...
result_table<-function(obj,var_treat,var_weight,cov,cov_type,data,estimator){
  ...
  if(estimator=="ATE"){
    robust_var<-sandwich_var_ATE(obj,data)
    result_table[4]<-sqrt(robust_var[2,2])
  }
  else if(estimator=="ATT"){
    robust_var<-sandwich_var_ATT(obj,data)
