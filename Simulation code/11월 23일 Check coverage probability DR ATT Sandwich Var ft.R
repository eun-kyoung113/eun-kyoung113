...
write.csv(A,file="A_100.csv",row.names=FALSE)
...
write.csv(A,file="A_1000.csv",row.names=FALSE)
...
### Data store ------------------------------------------------------
data_L<-as.data.frame(cbind("A"=A,"B"=B,"C"=C,"Y"=Y))
colnames(data_L)<-c("A",  "B",  "C",  "Y")

### - # of obs = 100 -----------------------------------------
A_100<-read.csv("A_100.csv")
...
for(rep in 1:REPL){
  ...
  D_hat<-sqrt(cov_hat[1,1]+cov_hat[2,2]-2*cov_hat[1,2])
  
  DR_ATT<-DR_estimator("ATT",data,var_treat,var_y,cov)
  
  est_DR_100[rep]<-DR_ATT$est
  est_100_CV[rep,1]<-est_DR_100[rep]-1.96*(sqrt(D_hat)/sqrt(nrow(data)))
  est_100_CV[rep,2]<-est_DR_100[rep]+1.96*(sqrt(D_hat)/sqrt(nrow(data)))
...
### -------------------------------------------------------------------------
### D(theta_hat) --------------------------------------------------------
est_DRstar_100<-rep(NA,length=REPL)

eststar_100_CV<-matrix(NA,nrow=REPL,ncol=3)
colnames(eststar_100_CV)<-c("CL","CU","contain")
...
for(rep in 1:REPL){
  ...
  j_hat<-J_function(data,data_L,var_treat,var_y,cov)
  crossprod<-crossprod_est_equ(data,data_L,var_treat,var_y,cov)
  
  cov_true_hat<-(solve(j_hat)%*%crossprod%*%t(solve(j_hat)))
  D_true_hat<-sqrt(cov_true_hat[1,1]+cov_true_hat[2,2]-2*cov_true_hat[1,2])
  
  DR_ATT<-DR_estimator("ATT",data,var_treat,var_y,cov)
  
  est_DRstar_100[rep]<-DR_ATT$est
  eststar_100_CV[rep,1]<-est_DRstar_100[rep]-1.96*(sqrt(D_true_hat)/sqrt(nrow(data_L)))
  eststar_100_CV[rep,2]<-est_DRstar_100[rep]+1.96*(sqrt(D_true_hat)/sqrt(nrow(data_L)))
  eststar_100_CV[rep,3]<-ifelse((eststar_100_CV[rep,1]<=log(2) & eststar_100_CV[rep,2]>=log(2)), 1, 0)
  
}

D_star_theta_hat_coverage_100<-mean(eststar_100_CV[,3])
...
for(rep in 1:REPL){
  ...
  est_1000_CV[rep,1]<-est_DR_1000[rep]-1.96*(sqrt(D_hat)/sqrt(nrow(data)))
  est_1000_CV[rep,2]<-est_DR_1000[rep]+1.96*(sqrt(D_hat)/sqrt(nrow(data)))
...
### -------------------------------------------------------------------------
### D(theta_hat) --------------------------------------------------------
est_DRstar_1000<-rep(NA,length=REPL)

eststar_1000_CV<-matrix(NA,nrow=REPL,ncol=3)
colnames(eststar_1000_CV)<-c("CL","CU","contain")
...
for(rep in 1:REPL){
  ...
  est_DRstar_1000[rep]<-DR_ATT$est
  eststar_1000_CV[rep,1]<-est_DRstar_1000[rep]-1.96*(sqrt(D_true_hat)/sqrt(nrow(data_L)))
  eststar_1000_CV[rep,2]<-est_DRstar_1000[rep]+1.96*(sqrt(D_true_hat)/sqrt(nrow(data_L)))
  eststar_1000_CV[rep,3]<-ifelse((eststar_1000_CV[rep,1]<=log(2) & eststar_1000_CV[rep,2]>=log(2)), 1, 0)
  
}

D_star_theta_hat_coverage_1000<-mean(eststar_1000_CV[,3])
