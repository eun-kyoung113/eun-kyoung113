# Simulation code 10/14 Version --------------------------------------------
### <What to Do> ------------------------------------------------------------
## 1. Debugging Sandwich Variance Estimator function of DR ATE Estimator --- 잘못된 부분 수정
### -------------------------------------------------------------------------
...
### 3-1) Data generation --------------------------------------------------
### - with # of obs = 100000 -----------------------------------------
B_sample<-matrix(0,nrow=100000,ncol=1)
C_sample<-matrix(0,nrow=100000,ncol=1)
U_sample<-matrix(0,nrow=100000,ncol=1)
E_sample<-matrix(0,nrow=100000,ncol=1)
Y_sample<-matrix(0,nrow=100000,ncol=1)
...
### Data store ---------------------------------------------------------
write.csv(B_sample,file="B_sample_100000_true.csv",row.names=FALSE)
write.csv(C_sample,file="C_sample_100000_true.csv",row.names=FALSE)
#write.csv(U_sample,file="U_sample.csv",row.names=FALSE)
write.csv(E_sample,file="E_sample_100000_true.csv",row.names=FALSE)
write.csv(Y_sample,file="Y_sample_100000_true.csv",row.names=FALSE)


### 3-1-2) Data import -------------------------------------------------------------
B_sample<-read.csv("B_sample_100000_true.csv",header=TRUE)
C_sample<-read.csv("C_sample_100000_true.csv",header=TRUE)
#U_sample<-read.csv("U_sample.csv",header=TRUE)
E_sample<-read.csv("E_sample_100000_true.csv",header=TRUE)
Y_sample<-read.csv("Y_sample_100000_true.csv",header=TRUE)
...
nu_true_array<-array(0,dim=c(dim1,dim1,nrow(data)))

for(j in 1:nrow(data)){
  
  est_1_true<-(log(1.5)*0.5 + log(2))-((data$E[j]*data$Y[j]-(data$E[j]-true_ps[j])*(as.vector(c(0, true_effect)) %*% X_1[j,]))/true_ps[j])
  est_2_true<-(log(1.5)*0.5)-(((1-data$E[j])*data$Y[j]+(data$E[j]-true_ps[j])*(as.vector(c(0, true_effect))%*%X_0[j,]))/(1-true_ps[j]))
  est_3_true<-model.matrix(DR_ATE$out_model)[j,]*as.vector(data$Y[j] - as.vector(true_effect)%*%t(as.matrix(data[j,c("E",cov)])))
  est_4_true<-(data$E[j]-true_ps[j])*model.matrix(fit)[j,]
...
### 3-3) J_theta true value & hat value approximation ------------------------------------------
...
for(j in 1:nrow(data)){
  nu1_3<-matrix(rep(0,length(coef(DR_ATE$out_model))),nrow=1)         
  nu1_4<-matrix(rep(0,length(coef(fit))),nrow=1)
  nu1<-cbind(1,0,nu1_3,nu1_4)
  colnames(nu1)<-NULL
  
  nu2_3<-(nrow(data)/sum(data[,var_treat]))*(matrix(((-(1+exp_beta_X_true[j]))*data[,var_treat][j]+exp_beta_X_true[j])*X_0[j,],nrow=1))
  nu2_4<-(nrow(data)/sum(data[,var_treat]))*(as.vector((-1)*exp_beta_X_true[j]*((1-data[,var_treat][j])*(data[,var_y][j]-(true_effect%*%X_0[j,]))))*model.matrix(fit)[j,])
  nu2<-cbind(0,1,matrix(nu2_3,nrow=1),matrix(nu2_4,nrow=1))
  colnames(nu2)<-NULL
  
  nu3_3<--model.matrix(DR_ATE$out_model)[j,]%*%t(model.matrix(DR_ATE$out_model)[j,])
  nu3_4<-matrix(0,nrow=length(true_effect),ncol=length(coef(fit)))
  nu3<-cbind(0,0,nu3_3,nu3_4)
  colnames(nu3)<-NULL
  
  nu4_3<-matrix(0,nrow=length(coef(fit)),ncol=length(true_effect))
  nu4_4<- -((true_ps[j])*(1-true_ps[j]))*(model.matrix(fit)[j,]%*%t(model.matrix(fit)[j,]))
  nu4<-cbind(0,0,nu4_3,nu4_4)
  colnames(nu4)<-NULL
  
  nu_true<-rbind(nu1,nu2,nu3,nu4)
  nu_true_array[,,j]<-nu_true
  
}

j_true<-apply(nu_true_array,c(1,2),mean)
...
for(j in 1:nrow(data)){
  ...
  nu2_3<-(nrow(data)/sum(data[,var_treat]))*(matrix(((-(1+exp_beta_X[j]))*data[,var_treat][j]+exp_beta_X[j])*X_0[j,],nrow=1))
  nu2_4<-(nrow(data)/sum(data[,var_treat]))*(as.vector((-1)*exp_beta_X[j]*((1-data[,var_treat][j])*(data[,var_y][j]-(coef(DR_ATE$out_model)%*%X_0[j,]))))*model.matrix(fit)[j,])
  nu2<-cbind(0,1,matrix(nu2_3,nrow=1),matrix(nu2_4,nrow=1))
  colnames(nu2)<-NULL
  
  nu3_3<--model.matrix(DR_ATE$out_model)[j,]%*%t(model.matrix(DR_ATE$out_model)[j,])
  nu3_4<-matrix(0,nrow=length(coef(DR_ATE$out_model)),ncol=length(coef(fit)))
  nu3<-cbind(0,0,nu3_3,nu3_4)
  colnames(nu3)<-NULL
  
  nu4_3<-matrix(0,nrow=length(coef(fit)),ncol=length(coef(DR_ATE$out_model)))
  nu4_4<- -((fit$fitted.values[j])*(1-fit$fitted.values[j]))*(model.matrix(fit)[j,]%*%t(model.matrix(fit)[j,]))
  nu4<-cbind(0,0,nu4_3,nu4_4)
  colnames(nu4)<-NULL
  
  nu_theta<-rbind(nu1,nu2,nu3,nu4)
  nu_theta_array[,,j]<-nu_theta
  
}

j_hat<-apply(nu_theta_array,c(1,2),mean)

# -------------------------------------------------------------------------
### 3-2-1) compare sample variance of Z_theta : Data import ---------------
...
### 3-2-1-1) 1000th Replication for comparison My function with package --------------
...
abs(nu_true_mean-nu_theta)

# -------------------------------------------------------------------------

### 4) Debugging Variance estimator of DR ATT estimator : J_theta part -----------
### 4-1) Data generating : #of obs = 1000-----------------------------------------
B_sample<-matrix(0,nrow=1000,ncol=1)
C_sample<-matrix(0,nrow=1000,ncol=1)
#U_sample<-matrix(0,nrow=1000,ncol=1)
E_sample<-matrix(0,nrow=1000,ncol=1)
Y_sample<-matrix(0,nrow=1000,ncol=1)
...
estimating_equation_true <- as.matrix(rbind(est_1_true,est_2_true,t(est_3_true),t(est_4_true)))
one<-apply(estimating_equation_true,1,sum)/sqrt(nrow(data))
...
### - # of obs = 10000 -----------------------------------------
B_sample<-matrix(0,nrow=10000,ncol=1)
C_sample<-matrix(0,nrow=10000,ncol=1)
U_sample<-matrix(0,nrow=10000,ncol=1)
E_sample<-matrix(0,nrow=10000,ncol=1)
Y_sample<-matrix(0,nrow=10000,ncol=1)
