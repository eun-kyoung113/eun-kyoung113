### -------------------------------------------------------------------------
### Title : Variance Estimator of Monte Carlo Approximation version 2 -----------------
###       : give 1000th estimator
###       : for Scenario 4

### Purpose : Calculate SD Ratio --------------------------------------------
### Change : Change the process of data generating

Monte_result_4<-function(A_RATIO, A_VAR1, A_VAR2, A_VAR3, N, REPL){
  ...
  for(repl in 1:REPL){
    ...
    # outcome_data<-data[,c(var_y,var_treat,cov_outcome)]
    # outcome_reg_ATE<-lm(Y~., data=outcome_data)
    # 
    ...
    outcome_data<-data[,c(var_y,var_treat,cov_outcome)]
    outcome_reg_ATT<-lm(Y~., data=outcome_data)
